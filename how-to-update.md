...
10. 実機で設定統合、スイッチ保存、広告枠が消えて空間が詰まることを目視確認します。
11. ReVanced Managerへ`patches.json`をURL登録し、APK選択、パッチ完了、インストール直前まで確認します。
12. `git diff --check`、Git状態、秘密情報、生成物混入を確認してコミット・プッシュ・タグ作成します。
13. リリースのRVPと`patches.json`を取得し、ローカル生成物と同じ構造であることを再確認します。
...
- 署名が異なるAPKを無理に上書きしません。rootマウントを使った場合は、Managerの復旧手順で元のシステムAPKへ戻します。
